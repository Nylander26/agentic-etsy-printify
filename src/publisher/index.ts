...
/**
 * Groups approved designs by the CONCEPT they render and keeps only the best `max`
 * variations of each.
 *
 * The generator produces `generation.variations_per_design` treatments (base / dark /
 * no-text) of every design idea. Drafting all of them ships near-identical listings whose
 * SEO tags are generated from the same concept, so they compete against each other for the
 * same queries — and Etsy rarely surfaces two listings from one shop in a single result
 * set. Confirmed live on LaughLoudDesigns: 14 listings covering only 5 concepts.
 *
 * "Best" = highest validator overall score; ties fall back to the `base` treatment, then
 * to the design id so the pick is deterministic across runs (a run must not re-draft a
 * different variation of a concept it already published).
 */
export function selectVariationsPerConcept(
  designs: ApprovedDesign[],
  max: number
): ApprovedDesign[] {
  const VARIATION_RANK: Record<string, number> = { base: 0, dark: 1, "no-text": 2, minimal: 3 };

  const byConcept = new Map<string, ApprovedDesign[]>();
  for (const d of designs) {
    // Concept text is shared by every variation of one idea; niche keeps two niches from
    // colliding on a generic concept. Fall back to the design id so a design with no
    // concept text is never silently merged into another's group.
    const concept = d.meta.concept?.trim().toLowerCase();
    const key = concept ? `${d.meta.niche}\u0000${concept}` : `id\u0000${d.meta.id}`;
    const group = byConcept.get(key);
    if (group) group.push(d);
    else byConcept.set(key, [d]);
  }

  const kept: ApprovedDesign[] = [];
  for (const group of byConcept.values()) {
    const ranked = [...group].sort((a, b) => {
      const scoreA = a.meta.validation?.scores.overall ?? 0;
      const scoreB = b.meta.validation?.scores.overall ?? 0;
      if (scoreB !== scoreA) return scoreB - scoreA;
      const rankA = VARIATION_RANK[a.meta.variation] ?? 99;
      const rankB = VARIATION_RANK[b.meta.variation] ?? 99;
  ...
  // Preserve the caller's original ordering so the per-run cap still slices predictably.
  const keptIds = new Set(kept.map((d) => d.meta.id));
  return designs.filter((d) => keptIds.has(d.meta.id));
}
...
async function draftDesign(
  ...
  // The margin every draft ships with was never printed — a bad target_net_margin or a
  // garment cost change showed up only as a different number in the Printify dashboard.
  console.log(
    `    Precio: $${pricing.suggestedPrice.toFixed(2)} · margen neto ` +
      `${pricing.margin.toFixed(1)}% ($${pricing.marginUSD.toFixed(2)}/venta)` +
      (pricing.warning ? ` ⚠️ ${pricing.warning}` : "")
  );
...
export async function publishApproved(): Promise<{ drafted: number; failed: number; skipped: number }> {
  ...
  // Collapse near-duplicate variations of the same concept BEFORE the per-run cap, so the
  // cap budgets distinct concepts instead of being eaten by three treatments of one idea.
  const maxVariations = getConfig().publishing.max_variations_per_concept;
  const deduped = selectVariationsPerConcept(allApproved, maxVariations);
  if (deduped.length < allApproved.length) {
    console.log(
      `\n  ℹ️  ${allApproved.length - deduped.length} variaciones duplicadas omitidas ` +
        `(max_variations_per_concept=${maxVariations}) → ${deduped.length} conceptos únicos`
    );
  }
  ...
  const approved = deduped.slice(0, getConfig().publishing.max_publish_per_run);
